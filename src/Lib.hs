...
isGridInvalid grid =
  any isInvalidRow grid || any isInvalidRow (Data.List.transpose grid) || any
    isInvalidRow
    (subGridsToRows grid)
 where
  isInvalidRow row =
  ...
  hasDups l = hasDups' l []
  hasDups' [] _ = False
  hasDups' (y : ys) xs | y `elem` xs = True
...
solve grid = pruneGridSolver grid >>= solve'
 where
  solve' g
    ...
    = Just g
    | otherwise
    = let (grid1, grid2) = nextGrids g in solve grid1 <|> solve grid2
